...
def parse_args():
    ...
    parser.add_argument('--epochs', default=500, type=int, metavar='N',#ori:100
                        help='number of total epochs to run')
    parser.add_argument('-b', '--batch_size', default=8, type=int,#ori:16 256*25/8 = 800
    ...
    parser.add_argument('--input_channels', default=3, type=int,
                        help='input channels')
    parser.add_argument('--num_classes', default=10, type=int, #ori:1
    ...
    # loss
    parser.add_argument('--loss', default='DiceLoss_ace',# ori:BCEDiceLoss
                        choices=LOSS_NAMES,
                        help='loss: ' +
                        ' | '.join(LOSS_NAMES) +
                        ' (default: BCEDiceLoss)')
    ...
    # dataset
    parser.add_argument('--dataset', default='dhcp', #ori:isic
                        help='dataset name')
...
def train(config, train_loader, model, criterion, optimizer):
    avg_meters = {'loss': AverageMeter()} #ori:{'loss': AverageMeter(),'iou': AverageMeter()}
    ...
    for sample in train_loader:
        ...
        if config['deep_supervision']:
            ...
            for output in outputs:
                loss += criterion(output, target.long()) #ori:(output,target) for all code, change 4 dtype for target
            loss /= len(outputs)
            #ori: iou,dice = iou_score(outputs[-1], target) # 4 places are commented out 
        else:
            output = model(input)
        ...
        postfix = OrderedDict([
            ('loss', avg_meters['loss'].avg) #ori:('loss', avg_meters['loss'].avg),('iou', avg_meters['iou'].avg),
        ])
        pbar.set_postfix(postfix)
        pbar.update(1)
    pbar.close()

    return OrderedDict([('loss', avg_meters['loss'].avg)]# ori:add ('iou', avg_meters['iou'].avg)]
                        )
...
def score_perclass(vol_output, ground_truth, num_classes):#N,1,256,256
...
def main():
    ...
    elif config['loss'] == 'DiceLoss_ace': 
        #when use DiceLoss_ace, need to change dtype of target from torch.int32 to torch.int64  #output.dtype:torch.float32,
    ...
    NUM_CLASS = 10 
    NUM_SLICES = 1 #1
    DATA_DIR = '/opt/data/private/ACEnet_dhcp/resampled_Swin_UNet/'#############rewrite: train325,val25
    DATA_LIST = '/opt/data/private/ACEnet_dhcp/datasets/'
    train_dataset = LoadMRIData(DATA_DIR, DATA_LIST, 'train', NUM_CLASS, num_slices=NUM_SLICES, se_loss = False, use_weight = False, Encode3D=True)
    train_loader = torch.utils.data.DataLoader(
        ...
        drop_last=True)
    #val_dataset use the same dataset as train_dataset
    val_dataset = LoadMRIData(DATA_DIR, DATA_LIST, 'train', NUM_CLASS, num_slices=NUM_SLICES, se_loss = False, use_weight = False, Encode3D=True)
